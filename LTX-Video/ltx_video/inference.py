...
def load_image_to_tensor_with_resize_and_crop(
    ...
    image = image.crop((x_start, y_start, x_start +
                       new_width, y_start + new_height))
    if not just_crop:
    ...
    frame_tensor = frame_tensor_hwc.permute(
        2, 0, 1) * 255.0  # (H, W, C) -> (C, H, W)
...
def create_ltx_video_pipeline(
    ...
    if sampler == "from_checkpoint" or not sampler:
        ...
        scheduler = RectifiedFlowScheduler(
            sampler=("Uniform" if sampler.lower() ==
                     "uniform" else "LinearQuadratic")
        )
...
def load_pipeline_config(pipeline_config: str):
    ...
    if os.path.isfile(current_file.parent / pipeline_config):
        ...
        raise ValueError(
            f"Pipeline config file {pipeline_config} does not exist")
...
def infer(config: InferenceConfig):
    ...
    if conditioning_media_paths:
        ...
        if any(s < 0 or s > 1 for s in conditioning_strengths):
            raise ValueError(
                "All conditioning strengths must be between 0 and 1")
    ...
    if pipeline_config.get("pipeline_type", None) == "multi-scale":
        ...
        pipeline = LTXMultiScalePipeline(
            pipeline, latent_upsampler=latent_upsampler)
...
def prepare_conditioning(
    ...
    for path, strength, start_frame in zip(
        ...
        conditioning_items.append(ConditioningItem(
            media_tensor, start_frame, strength))
